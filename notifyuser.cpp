// Adapted from: https://codevlog.com/gmailsmtp-gmail-com-using-c-programming-ssl/118
...
char * NotifyUser::MailHeader(const char* from, const char* to, const char* subject, const char* mime_type, const char* charset) {
    ...
    //char Sender[6 + my_strlen(from) + 1 + 1];
    char Sender[] = "FROM: system<cs3307eyespy@gmail.com>\r\n";
//    char Recip[4 + my_strlen(to) + 1 + 1];
	char Recip[] = "To: User<cs3307eyespy@gmail.com>\r\n";
//    char Subject[8 + 1 + my_strlen(subject) + 1 + 1];
	char Subject[] = "Subject: EyeSpy Has Detected Motion\r\n";
//    char mime_data[13 + 1 + 3 + 1 + 1 + 13 + 1 + my_strlen(mime_type) + 1 + 1 + 8 + my_strlen(charset) + 1 + 1 + 2];
	char mime_data[] = "MIME-Version: 1.0\r\nContent-type: text/plain; charset=US-ASCII\r\n\r\n";

    strftime(date_buff, (33), "%a , %d %b %Y %H:%M:%S", localtime(&now));

    sprintf(Branding, "DATE: %s\r\nX-Mailer: %s\r\n", date_buff, app_brand);
//    Sender = "system<cs3307eyespy@gmail.com>\r\n";
...
//    memcpy(&mail_header[0 + my_strlen(Branding) + my_strlen(Sender) + my_strlen(Recip) + my_strlen(Subject)], &mime_data, my_strlen(mime_data));
    return mail_header;
}
